...
tmux のステータスバーは画面上部に二段で表示されます。上段にはセッション一覧と日時、下段には現在のセッションにあるウィンドウ一覧が並びます。現在選択しているセッションとウィンドウは反転・太字で表示されます。

セッション名またはウィンドウ名をクリックすると、その項目へ移動できます。マウス操作を含む設定は Home Manager で管理しているため、変更は次のコマンドで反映します。
